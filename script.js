// script.js - نسخه تمیز و بدون TTS (2025)
...
// بستن با کلیک روی دکمه‌های close
...
function renderCharacterPage() {
  ...
  document.getElementById('page-title').textContent = char.name[currentLang] + ' - بانک حکایت';
...
function updateSidebar() {
  ...
  list.innerHTML = storiesData.characters.map(char => `
    <li>
      <a href="character.html?id=${char.id}" class="${char.id === currentCharId ? 'bg-amber-500 text-white' : 'hover:bg-gray-100 dark:hover:bg-gray-800'} block px-6 py-4 rounded-xl transition font-medium">
        ${char.name[currentLang]}
      </a>
    </li>
  `).join('');
...
document.getElementById('stories-search')?.addEventListener('input', e => {
  const term = e.target.value.toLowerCase();
  const cards = document.querySelectorAll('#stories-list > div');
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if (!response.ok) throw new Error('فایل stories.json پیدا نشد');
    storiesData = await response.json();
    renderAll();

    // راه‌اندازی انیمیشن AOS
    ...
    document.body.insertAdjacentHTML('beforeend', `
      <div class="text-center py-20 text-red-600 dark:text-red-400 text-2xl">
        خطا در بارگذاری حکایات. لطفاً فایل stories.json را بررسی کنید.
      </div>
    `);
  }
});