// books-sidebar.js - نسخه 2025 با جستجوی زنده و هوشمند (فارسی + انگلیسی)
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // عناصر جستجو (باید در HTML وجود داشته باشه)
  ...
  langBtn?.addEventListener("click", () => {
    ...
    performSearch(); // دوباره جستجو با زبان جدید
  });
    ...
    if (!query) {
      ...
      filteredBooks = allBooks
        .map(book => {
          ...
          const matchesInBook = title.includes(query) ||
                                desc.includes(query) ||
                                matchingStories.length > 0;

          if (matchesInBook) {
            // اگر فقط بعضی فصل‌ها مطابقت دارن، فقط اونا رو نگه دار
  ...
  function updateResultsCount(count) {
    if (resultsCount) {
      resultsCount.textContent = count === allBooks.length
        ? ""
        : t[currentLang].resultsFound(count);
  ...
  // رندر کتاب‌ها (همان قبلی با کمی بهبود)
  ...
  // باز کردن مودال (بدون تغییر)
  window.openBookModal = function (book) { /* همان کد قبلی */ 
    ...
    (book.stories || []).forEach((story, idx) => {
      ...
      // هایلایت کردن متن جستجو شده (اختیاری و پیشرفته)
      ...
      chapter.className = "border-b border-gray-200 dark:border-gray-700 pb-12 last:border-0";
